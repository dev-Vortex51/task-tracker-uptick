...
function App() {
  ...
  const { todoTask, progressTask, doneTask } = useMemo(() => {
    return {
      todoTask: tasks.filter((task) => task.status === "todo"),
      progressTask: tasks.filter((task) => task.status === "in progress"),
      doneTask: tasks.filter((task) => task.status === "done"),
    };
  }, [tasks]);
